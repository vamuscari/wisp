...
fn registry_entries_with_duplicate_fields_are_discarded_before_use() {
    ...
    fs::write(
        &path,
        format!(
            r#"{{"registry_version":7,"registry_version":7,"instance_id":"1:/repos/wisp","pid":1,"server_url":"{}","directory":"/repos/wisp","project_path":"/repos/wisp","updated_at":{},"session_waiting":{{"permissions":0,"questions":0}}}}"#,
            server.url(),
            now_millis()
        ),
    )
...
fn plugin_registration_is_versioned_atomic_updatable_and_removable() {
    ...
    assert_eq!(first["registry_version"], 7);
